...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function(num){
    const average = this.rating + num /2
    this.rating = average
    return average
  }
  ...
  // const newUser = new User('Jake', 33, 'jake.lane0312@gmail.com', [])

  // console.log(newUser)
  // newUser.addSavedPost('Jake2', 'boss', 100)
  ...
  User.prototype.removeSavedPost = function (id){
    this.savedPosts = this.savedPosts.filter(function(post){
      if (post.id === id){
        return false;
      }else return true;
    })
  ...
  User.prototype.changePostRating = function (num1,num2){
    let index = this.savedPosts.findIndex(function(post){
      if(post.id === num1){
      return true;}
    })

    let newObj = {...this.savedPosts[index], rating: num2}

    
    
    this.savedPost = this.savedPosts.splice(index,1,newObj)
  }
